...
let argDefs = 
  ...
  ("-linear", Arg.Set linear, "Use the pi calculus without replication")]
...
let main () = 
  let inFile, linear = parseArgs() in
  let ctx = emptycontext in
  Printf.printf "linear is %b" linear;
  let _ = process_file inFile (setlinear ctx linear) in
  ()
